...
- 온보딩 화면
  - 서비스 핵심 질문과 가치 제안을 처음 전달하는 시작 화면
- 회원가입 및 로그인 화면
  - 사업계획서 MVP에 포함된 회원 진입 화면
- 홈 화면
  - 오늘의 투자 메모와 AI 추천 카드가 가장 먼저 보이는 핵심 화면
- 포트폴리오 화면
  - 사용자가 현재 적립 중인 종목을 관리하는 화면
- 종목 검색 화면
  - 회사명, 티커, 거래소, 현재가를 보고 종목을 선택하는 화면
- 종목 선택 후 등록 화면
  - 종목명과 티커는 자동 입력되고, 사용자는 적립 금액, 보유 수량, 시작일, 메모만 입력하는 화면
- 추천 화면
  - 모든 종목의 추천 상태를 한 번에 모아보는 화면
- 추천 근거 화면
  - 주가/뉴스/기관/실적/밸류에이션/AI 의견을 보여주는 핵심 화면
- 종목 상세 화면
  - 차트, 뉴스, 기관 활동, 투자자 활동, AI 추천을 통합한 상세 화면
- 설정 화면
  - 알림, 추천 엔진 버전, API 연동 상태를 관리하는 화면
- 화면 설계도 화면
  - 위 화면들을 제품 구조 관점에서 빠르게 검토하는 내부용 허브 화면
