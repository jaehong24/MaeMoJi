...
현재 구현에서는 가격을 `PRICE_MOMENTUM`, `PRICE_STABILITY` 두 하위 축으로 나누어 계산한다.
또한 `수익성`과 `재무건전성`은 재무제표 기반 세부 점수를 만들고,
화면 표시에서는 당분간 `기업 체력` 묶음으로도 함께 설명 가능하게 유지한다.

즉 운영 기준 최종 구조는 아래와 같다.
...
| `VALUATION` | 현재 가격 부담과 기대 반영 정도 | 0~100 | 12 | `stock_price_snapshots`, 지표 배치 |
| `QUALITY_OF_GROWTH` | 성장의 지속 가능성과 성장 대비 품질 | 0~100 | 12 | `stock_price_snapshots`, 지표 배치 |
| `PROFITABILITY` | 마진, ROE, ROIC, EPS 기반 수익 체력 | 0~100 | 내부 하위 축 | 재무지표 배치 |
| `BALANCE_SHEET` | 부채, 유동성, 현금흐름 기반 재무건전성 | 0~100 | 내부 하위 축 | 재무지표 배치 |
| `USER_FIT` | 사용자의 보유 상황과 투자 부담 적합성 | 0~100 | 8 | `portfolio_items` |

비고:

- `PROFITABILITY`, `BALANCE_SHEET`은 현재 `FUNDAMENTAL_QUALITY`의 내부 세부 축이지만,
  모델 설계상 최종 판단 축으로 명시적으로 관리한다.
- `FUNDAMENTAL_QUALITY`는 수익성, 안정성, 현금흐름, 효율성을 묶은 종합 체력 개념으로 유지한다.
...
1. `기업 체력 높음 + 가격 과열 높음 = 증액 제한`
   - 좋은 회사라도 단기 과열이면 바로 증액으로 밀지 않는다.
   - 강한 펀더멘털은 유지하되, 추격 매수 부담을 별도 패널티로 반영한다.

2. `뉴스 좋음 + 밸류에이션 부담 큼 = 유지 우선`
   - 뉴스가 단기적으로 좋아도 비싼 가격이면 증액을 쉽게 주지 않는다.
   - 호재 뉴스는 반영하되, 이미 기대가 반영된 구간으로 해석한다.

3. `성장 둔화 + 밸류에이션 부담 큼 = 감액 강화`
   - 성장의 질이 약해지는데 가격까지 비싸면 가장 보수적으로 본다.
   - 이 경우는 `REDUCE` 쪽으로 기울도록 교차 패널티를 더 강하게 준다.
